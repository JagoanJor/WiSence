...
namespace API.Controllers
{
    ...
    {
        private IServiceAsync<Calendar> _service;
        private IAuthService _authService;

        public CalendarController(IServiceAsync<Calendar> service, IAuthService authService)
        {
            _service = service;
        ...
        public async Task<IActionResult> Get(int limit = 0, int page = 0, string search = "", string sort = "", string filter = "", string date = "")
        {
            try
            {
                ...
                var access = _authService.CheckRoleAccesibility(user.RoleID, "Perusahaan - Kalender");
                if (!access.IsRead)
        ...
        [Authorize]
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] Calendar obj)
        {
            try
            {
                var token = HttpContext.Request.Headers["Authorization"].FirstOrDefault()?.Split(" ").Last();
        ...
        [Authorize]
        [HttpPut]
        public async Task<IActionResult> Edit([FromBody] Calendar obj)
        {
            try
            {
                var token = HttpContext.Request.Headers["Authorization"].FirstOrDefault()?.Split(" ").Last();

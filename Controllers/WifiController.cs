    ...
    public class WifiController : ControllerBase
    {
        ...
        public IActionResult Get(int limit = 0, int page = 0, string search = "", string sort = "", string filter = "", string date = "")
        {
            try
            {
                ...
                var response = new ListResponse<Wifi>(result, total, page);
                return Ok(response);
            }
            catch (Exception ex)
            {
                ...
                Trace.WriteLine(message, "WifiController");
        ...
        [Authorize]
        [HttpPost]
        public IActionResult Create([FromBody] Wifi obj)
        {
            try
            {
        ...
        [Authorize]
        [HttpPut]
        public IActionResult Edit([FromBody] Wifi obj)
        {
            try
            {
                var token = HttpContext.Request.Headers["Authorization"].FirstOrDefault()?.Split(" ").Last();

...
namespace API.Controllers
{
    ...
    {
        private IService<Vendor> _service;

        public VendorController(IService<Vendor> service)
        {
            _service = service;
        }
        ...
        public IActionResult Get(int limit = 0, int page = 0, string search = "", string sort = "", string filter = "", string date = "")
        {
            try
            {
                ...
                var response = new ListResponse<Vendor>(result, total, page);
                return Ok(response);
            }
            catch (Exception ex)
            {
        ...
        public IActionResult GetById(int id)
        {
            try
            {
                ...
                var response = new Response<Vendor>(result);
                return Ok(response);
            }
            catch (Exception ex)
            {
        ...
        [Authorize]
        [HttpPost]
        public IActionResult Create([FromBody] Vendor obj)
        {
            try
            {
        ...
        [Authorize]
        [HttpPut]
        public IActionResult Edit([FromBody] Vendor obj)
        {
            try
            {
                var token = HttpContext.Request.Headers["Authorization"].FirstOrDefault()?.Split(" ").Last();

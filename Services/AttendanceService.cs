    ...
    public class AttendanceService : IAttendanceService
    {
        ...
        public bool Delete(Int64 id, String userID)
        {
            ...
            try
            {
                ...
                obj.IsDeleted = true;
                obj.UserUp = userID;
                obj.DateUp = Utils.ConvertToIndonesianTime(DateTime.UtcNow).AddMinutes(-2);
        ...
        /*public Attendance ClockIn(User user)
        {
            ...
            try
            {
                ...
                var attendance = context.Attendances.FirstOrDefault(x => x.UserID == user.UserID && x.IsDeleted != true && x.Date.Value.Date == Utils.ConvertToIndonesianTime(DateTime.UtcNow).Date && (x.Status == "Cuti" || x.Status == "WFH"));
                if (attendance != null)
                ...
                if (Utils.ConvertToIndonesianTime(DateTime.UtcNow).TimeOfDay < shift.ClockIn.Value.AddMinutes(-30).TimeOfDay)
                ...
                data.ClockIn = Utils.ConvertToIndonesianTime(DateTime.UtcNow);
                data.UserIn = user.UserID.ToString();
                data.DateIn = Utils.ConvertToIndonesianTime(DateTime.UtcNow);
                data.Date = Utils.ConvertToIndonesianTime(DateTime.UtcNow);
                data.IsDeleted = false;

                DateTime lateTime = shift.ClockIn.Value.AddHours(1).AddMinutes(30);

                if (Utils.ConvertToIndonesianTime(DateTime.UtcNow).TimeOfDay <= shift.ClockIn?.TimeOfDay)
                    data.Status = "Ontime";
                else if (Utils.ConvertToIndonesianTime(DateTime.UtcNow).TimeOfDay >= shift.ClockIn?.TimeOfDay && Utils.ConvertToIndonesianTime(DateTime.UtcNow).TimeOfDay < lateTime.TimeOfDay)
        ...
        /*public Attendance ClockOut(User user)
        {
            ...
            try
            {
                ...
                var data = context.Attendances
                    .Where(x => x.UserID == user.UserID && x.Date.Value.Date == Utils.ConvertToIndonesianTime(DateTime.UtcNow).Date && x.IsDeleted != true)
                    .FirstOrDefault();
                ...
                data.ClockOut = Utils.ConvertToIndonesianTime(DateTime.UtcNow);
                data.UserUp = user.UserID.ToString();
                data.DateUp = Utils.ConvertToIndonesianTime(DateTime.UtcNow);
        ...
        public void CheckAttendance(Int64? userID)
        {
            ...
            while (currentDate.Date < Utils.ConvertToIndonesianTime(DateTime.UtcNow).Date)
            {
                ...
                if (currentDate.DayOfWeek.ToString() != "Saturday" && currentDate.DayOfWeek.ToString() != "Sunday")
                {
                    ...
                    if (holiday == null)
                    {
                        if (haveAttend == null)
                        {
                            ...
                            attendance.DateIn = Utils.ConvertToIndonesianTime(DateTime.UtcNow);
